        ...
        {topics.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {topics.map((topic: any) => (
              <div key={topic.id} className="bg-white rounded-2xl shadow p-6 border border-gray-200 hover:shadow-md transition cursor-pointer">
                ...
                <a href={`/topics/${topic.slug}`} className="block text-center bg-brand-700 hover:bg-brand-600 text-white py-3 rounded-lg font-semibold transition">
                  Start Topic
                </a>
              </div>
            ))}
